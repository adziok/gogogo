<script lang="ts">
	import type { FeatureFlag, UpdateFlagPayload } from '$lib/types';
	...
	let {
		...
		open: boolean;
		onClose: () => void;
		onUpdate: (id: string, payload: UpdateFlagPayload) => Promise<void>;
	} = $props();
	...
	async function submit(e: Event) {
		...
		submitting = true;
		try {
			await onUpdate(flag.id, { description: description.trim(), enabled });
		} finally {
			submitting = false;
		}
	}
		...
		<div class="modal">
			...
			<form onsubmit={submit}>
				...
				<div class="field checkbox">
					<input id="edit-enabled" type="checkbox" bind:checked={enabled} disabled={submitting} />
...
<style>
	...
	.flag-name { color: #495057; font-weight: 600; font-size: 0.9rem; margin-bottom: 1rem; }
	...
	.btn-secondary:hover:not(:disabled) { background: #de2e6; }
</style>